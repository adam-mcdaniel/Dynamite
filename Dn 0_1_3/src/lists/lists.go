...
func Pop(vm *VirtualMachine) {
	...
	vm.Push(self)
	Length(vm)
	length := vm.Pop().(float64)
	_, ok := self[strings.Repeat("a", int(length))]
	if ok {
		delete(self, strings.Repeat("a", int(length)))
		self["counter"] = strings.Repeat("a", int(length))
	}
	vm.Push(self)
}
...
func Items(vm *VirtualMachine) {
	...
	vm.Push(self)
	Length(vm)
	limit := vm.Pop().(float64)
	for i := 1; i < int(limit)+1; i++ {
		vm.Push(self)
